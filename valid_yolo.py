...
class Yolo_verification:
    ...
    def validation(self):
        for i in tqdm(range(len(self.img_names))):
            ...
            Image.fromarray(patched_img.detach().cpu().numpy().astype("uint8")).save("./gen_results/"+self.img_names[i][:-4]+".png")
...
def main():
    mask = cv2.imread("./submission/pgd_half2/mask.png")
    ...
    with open("./datasets/loc.json",'r') as f:
        ...
        patch = torch.from_numpy(np.array(Image.open("./submission/pgd_half2/pgd_smooth_epoch16.png")).astype(np.float32))
