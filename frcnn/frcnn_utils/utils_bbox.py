...
class DecodeBox():
    ...
    def frcnn_correct_boxes(self, box_xy, box_wh, input_shape, image_shape):
        ...
        #-----------------------------------------------------------------#
        box_yx = box_xy[:, torch.tensor([1,0])]
        box_hw = box_wh[:, torch.tensor([1,0])]
        input_shape = np.array(input_shape)
        image_shape = np.array(image_shape)

        box_mins    = box_yx - (box_hw / 2.)
        box_maxes   = box_yx + (box_hw / 2.)
        boxes  = torch.concat([box_mins[:, 0:1], box_mins[:, 1:2], box_maxes[:, 0:1], box_maxes[:, 1:2]], -1)
        boxes *= torch.concat([torch.from_numpy(image_shape), torch.from_numpy(image_shape)], -1).cuda()
        return boxes
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou = 0.3, confidence = 0.5):
        ...
        for i in range(bs):
            ...
            if len(results[-1]) > 0:
                results[-1] = [i.unsqueeze(0) for i in results[-1]]
                results[-1] = torch.concat(results[-1], 0)
